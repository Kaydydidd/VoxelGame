...
namespace {
    ...
    struct HeightTile {
        // Cooperative work-stealing fill state.
        std::atomic<int> nextRow{ 0 };    // next unclaimed row [0, REGION_DIM)
        std::atomic<int> doneRows{ 0 };    // rows whose data is fully written

        uint8_t h[REGION_DIM * REGION_DIM];

        bool    ready() const { return doneRows.load(std::memory_order_acquire) >= REGION_DIM; }
        uint8_t at(int lx, int lz) const { return h[lz * REGION_DIM + lx]; }
    };

    inline int64_t RegionKey(int rx, int rz) {
    ...
    struct HeightCache {
        ...
        uint32_t                                                 epoch = 0;   // tracks highest job epoch seen
    } hcache;
...
// -------------------------------------------------------------------
static void FillRegionHeights(int rx, int rz, HeightTile& t) {
    ...
    while ((lz = t.nextRow.fetch_add(1, std::memory_order_relaxed)) < REGION_DIM) {
        ...
        t.doneRows.fetch_add(1, std::memory_order_release);   // publish this row
    }

    // All rows are claimed; wait for any peer still finishing its last row.
...
// -------------------------------------------------------------------
static std::shared_ptr<const HeightTile> GetRegionHeights(int rx, int rz, uint32_t jobEpoch) {
    ...
    std::shared_ptr<HeightTile> tile;
    {
        ...
        // Signed diff so the comparison stays correct across uint32 wrap.
        const int32_t d = int32_t(jobEpoch - hcache.epoch);
        if (d < 0)                          // caller's job is stale – touch nothing
            return nullptr;
        if (d > 0) {                        // caller is from a newer epoch – retire old tiles
            hcache.tiles.clear();
    ...
    if (!tile->ready())
        FillRegionHeights(rx, rz, *tile);   // join the fill; returns only when complete
...
// ===================================================================
static std::unique_ptr<Chunk> BuildChunkTerrain(int cx, int cz, uint32_t jobEpoch) {
    const int  rx = ChunkToRegion(cx);
    const int  rz = ChunkToRegion(cz);
    const auto tile = GetRegionHeights(rx, rz, jobEpoch);
    if (!tile)
        return nullptr;                    // stale job detected via height cache – abandon
